...
submitButton.addEventListener('click', (event) => {
    ...
    if (cityName === '') {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=4885c4b997291cbbdb529b59964ffed7`)
...
getCurrentPosition.addEventListener('click', (event) => {
    ...
    function success(position) {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=4885c4b997291cbbdb529b59964ffed7`)
            .then(response => response.json())
            .then(weatherData => {
                displayWeatherDetails(weatherData);
...
function displayLocalStorageData() {
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${localStorageData}&appid=4885c4b997291cbbdb529b59964ffed7`)
        .then(response => response.json())
        .then(weatherData => {
            displayWeatherDetails(weatherData);
        })
...
function displayWeatherDetails(weatherData) {
    ...
    imageIcon.src = `http://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png`;
...
function mapfun(lat, lng) {
    mapboxgl.accessToken = 'pk.eyJ1IjoiZGlueXRob21hczg5IiwiYSI6ImNrbTNieW55YTBhOXMyd3Fkem41ejF5dDUifQ.zJuXJWcxBUw3owpSS9-bAA';
